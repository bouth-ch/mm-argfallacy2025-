"""
CLI: wavlm_roberta_afc with Whisper transcriptions as text input.

Identical to wavlm_roberta_afc but RoBERTa receives Whisper-transcribed text
instead of the original annotated snippet, so text and audio encode the same content.

From project root:
    python scripts/run_wavlm_roberta_afc_whisper.py

Background:
    nohup python scripts/run_wavlm_roberta_afc_whisper.py > logs/wavlm_roberta_afc_whisper.log 2>&1 &
"""
import sys
...
from src.data.whisper_dataset import WhisperMMUSEDFallacy
...
config['name'] = 'wavlm_roberta_afc_whisper'

loader = WhisperMMUSEDFallacy(
    task_name='afc',
    base_data_path=ROOT / 'data',
)

print(f"Starting experiment: {config['name']}")
print("Text  : RoBERTa-base — Whisper transcriptions")
